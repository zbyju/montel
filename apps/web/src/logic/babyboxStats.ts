...
export function calculateTotal(records: Record[]): StatsTotal {
  return records.reduce((acc, r) => {
    return r.sex === "M" ? [{ id: acc[0].id, value: acc[0].value + 1 }, acc[1]]
      : r.sex === "F" ? [acc[0], { id: acc[1].id, value: acc[1].value + 1 }]
        : acc
  }, [{ id: "Kluci", value: 0 }, { id: "Holky", value: 0 }])
}
...
export function calculateChildrenByDay(records: Record[]): StatsBy {
  return records.reduce((acc, r) => {
    ...
    return acc.map((d, i) => {
      if (di != i) return d
      return r.sex === "M" ? { ...d, Kluci: d.Kluci + 1 } : r.sex === "F" ? { ...d, Holky: d.Holky + 1 } : d
    })
  }, [{ x: "Pondělí", Kluci: 0, Holky: 0 }, { x: "Úterý", Kluci: 0, Holky: 0 }, { x: "Středa", Kluci: 0, Holky: 0 }, { x: "Čtvrtek", Kluci: 0, Holky: 0 }, { x: "Pátek", Kluci: 0, Holky: 0 }, { x: "Sobota", Kluci: 0, Holky: 0 }, { x: "Neděle", Kluci: 0, Holky: 0 }])
}
...
export function calculateChildrenByMonth(records: Record[]): StatsBy {
  ...
  }, ["Leden", "Únor", "Březen", "Duben", "Květen", "Červen", "Červenec", "Srpen", "Září", "Říjen", "Listopad", "Prosinec"].map(m => ({ x: m, Kluci: 0, Holky: 0 })))
}
...
export function calculateChildrenByHour(records: Record[]): StatsBy {
  ...
  }, Array.from(Array(24).keys()).map(h => ({ x: h, Kluci: 0, Holky: 0 }))).map(x => {
