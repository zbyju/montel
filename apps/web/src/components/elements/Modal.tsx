...
export default function Modal({ title, description, isOpen, onClose, className }: Props) {
  useEffect(() => {
    if (isOpen) {
  ...
  return (
    <>
      {isOpen ? (
        <div className={`${className} fixed top-0 left-0 w-full h-full`} onClick={onClose}>
          <div className="absolute inset-0 bg-gray-800 opacity-20" onClick={onClose}></div>
